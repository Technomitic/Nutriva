...
export function useDynamic() {
  ...
  return {
    isDark,
    // Backgrounds
    bg: isDark ? '#0D1B0F' : '#F5F7F5',
    cardBg: isDark ? 'rgba(20,40,22,0.7)' : 'rgba(255,255,255,0.85)',
    headerBg: isDark ? 'rgba(13,27,15,0.95)' : 'rgba(255,255,255,0.9)',
    // Text
    text: isDark ? '#E8F0E5' : '#1B3C12',
    textSecondary: isDark ? '#C8E6C9' : '#2E4A26',
    textMuted: isDark ? 'rgba(200,220,195,0.5)' : 'rgba(27,60,18,0.5)',
    textDim: isDark ? 'rgba(200,220,195,0.3)' : 'rgba(27,60,18,0.35)',
    // Accent
    accent: isDark ? '#66BB6A' : '#2E7D32',
    accentLight: isDark ? 'rgba(102,187,106,0.15)' : 'rgba(46,125,50,0.08)',
    // Borders
    border: isDark ? 'rgba(102,187,106,0.15)' : 'rgba(46,125,50,0.08)',
